...
# if (os.environ.get("CALC_PORT") == None):
#     port_to_use = args.port
# else:
#     port_to_use = port_to_use = args.port

# if (isinstance(os.environ.get("CALC_PORT"), int)):
#     port_to_use = os.environ.get("CALC_PORT")
# else:
#     port_to_use = args.port
...
try:
    ...
    f_handler = logging.FileHandler('bs_server.log')
...
try:
    port_to_use = int(os.getenv('CALC_PORT', '13337'))
except ValueError:
    logger.info("You have to put an integer in the CALC_PORT. Will use the default port 13337")
    port_to_use = 13337

if (port_to_use < 0 or port_to_use> 65535):
    logger.info("ERROR Le port spécifié n'est pas un port possible (de 0 à 65535).")
    sys.exit(1)
elif (port_to_use >= 0 and port_to_use<= 1024):
    logger.info("ERROR Le port spécifié est un port privilégié. Spécifiez un port au dessus de 1024.")
    sys.exit(2)
...
port = port_to_use # port choisi arbitrairement
